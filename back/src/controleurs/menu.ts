...
// - Voir le detail d'un menu (avec tracking MongoDB)
...
import { loggerConsultationMenu } from "../services/tracking";
...
// + Logge la consultation dans MongoDB pour les statistiques
// ============================================================
export async function detailMenu(req: Request, res: Response) {
  try {
    ...
    const utilisateurId = req.session.utilisateur?.id || null;
    loggerConsultationMenu(parseInt(id as string), utilisateurId);
  ...
  try {
    ...
    if (!titre || !nombrePersonnesMinimum || !prixParPersonne || !themeId) {
      ...
      return res.status(400).json({
        erreur:
          "Les champs titre, nombrePersonnesMinimum, prixParPersonne et themeId sont obligatoires",
      });
    }
  ...
  try {
    ...
    if (parseInt(commandesLiees.rows[0].nb) > 0) {
      return res.status(409).json({
        erreur:
          "Impossible de supprimer ce menu car il a des commandes associees",
      });
    }
